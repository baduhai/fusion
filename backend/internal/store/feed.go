...
func (s *Store) CreateFeed(groupID int64, name, link, siteURL, proxy string, refreshIntervalSeconds *int64) (*model.Feed, error) {
	...
	result, err := tx.Exec(`
		INSERT INTO feeds (group_id, name, link, site_url, proxy, refresh_interval_seconds)
		VALUES (:group_id, :name, :link, :site_url, :proxy, :refresh_interval_seconds)
	`, sql.Named("group_id", groupID), sql.Named("name", name), sql.Named("link", link),
		sql.Named("site_url", siteURL), sql.Named("proxy", proxy), sql.Named("refresh_interval_seconds", refreshIntervalSeconds))
	if err != nil {
		return nil, err
	}
